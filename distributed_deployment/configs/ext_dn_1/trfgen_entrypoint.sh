...
if [[ ${EBPF_GW_SETUP} == "yes" ]];then
  ...
  if [[ -n "$N6_IF_NAME" ]]; then
    ...
    echo
    echo -e "3. Add a route to UE subnet ($UE_IP_ADDRESS_POOL_1, $UE_IP_ADDRESS_POOL_2, $UE_IP_ADDRESS_POOL_3) via UPF N6 interface ($N6_UPF_IP_ADDR):"
    ip route add $UE_IP_ADDRESS_POOL_1 via $N6_UPF_IP_ADDR dev $N6_IF_NAME
    ip route add $UE_IP_ADDRESS_POOL_2 via $N6_UPF_IP_ADDR dev $N6_IF_NAME
    ip route add $UE_IP_ADDRESS_POOL_3 via $N6_UPF_IP_ADDR dev $N6_IF_NAME
  else
    echo
  ...
  iptables -t nat -A POSTROUTING -o $SGI_IF_NAME -s $UE_IP_ADDRESS_POOL_1 -j SNAT --to-source $GW_SGI_IP_ADDR
  iptables -t nat -A POSTROUTING -o $SGI_IF_NAME -s $UE_IP_ADDRESS_POOL_2 -j SNAT --to-source $GW_SGI_IP_ADDR
  iptables -t nat -A POSTROUTING -o $SGI_IF_NAME -s $UE_IP_ADDRESS_POOL_3 -j SNAT --to-source $GW_SGI_IP_ADDR
